...
describe('Testes de Integração', () => {
  ...
  const resultadoEsperadoGetClientes = [{
    Nome: 'João Silva',
    CPF: '000.000.000-00',
  },
  {
  ...
  test('TPM4', async () => {
    await db.cliente.create(clienteJoao);
    await db.cliente.create(clienteJoao1);
    await db.cliente.create(clienteJoao2);
    ...
    await request(app)
      ...
      .then((res) => {
        expect(res.body.erro).toBeUndefined();
        expect(res.status).toBe(200);
        expect(res.body).toMatchSnapshot(resultadoEsperadoGetClientes);
        expect(res.body).toMatchObject(resultadoEsperadoGetClientes);
      });
  });
});
